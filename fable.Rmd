 ...
 * Boosting, combining and ensemble forecasts
...
# fit1 <- TBATS(elecdemand, Demand)
fit2 <- ARIMA(elecdemand,
  Demand ~ Temperature + I(Temperature^2) + WorkDay)
summary(fit2)
#forecast(fit2) %>% autoplot()
# How to specify future regressors?
```
...
## Equivalent methods: forecast $\longrightarrow$ fable
  ...
  * `tslm`/`lm` $\longrightarrow$ `LM`
  * `tbats`  $\longrightarrow$ `TBATS`
  * `nnetar` $\longrightarrow$ `NNETAR`
  * `stlm` $\longrightarrow$ `STL %>% modelcomponents(ETS(seasadj),`\newline\hspace*{4.4cm}`SNAIVE(season))`
...
\fontsize{14}{15}\sf
  ...
  * `naive` $\longrightarrow$ `NAIVE %>% forecast`
  * `snaive` $\longrightarrow$  `SNAIVE %>% forecast`
  * `thetaf` $\longrightarrow$ `THETA` %>% `forecast`
  * `stlf` $\longrightarrow$ `STL %>% modelcomponents(ETS(seasadj),`\newline\hspace*{4.4cm}`SNAIVE(season)) %>% forecast`
  * `hw` $\longrightarrow$ `HW %>% forecast`
  * `holt` $\longrightarrow$ `HOLT %>% forecast`
  * `ses` $\longrightarrow$ `SES %>% forecast`
  * `splinef` $\longrightarrow$ `SPLINE %>% forecast`
  * `croston` $\longrightarrow$ `CROSTON %>% forecast`
