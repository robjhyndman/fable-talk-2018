...
cafe %>% ARIMA(log(value)) %>% forecast() %>% summary(level=90)
```
...
```{r auscafe7, echo=TRUE, dependson='auscafe2', fig.height=3.8}
cafe %>% ARIMA(log(value)) %>% forecast() %>% autoplot()
```
...
\begin{alertblock}{}
fable simplifies the time series model development process
\end{alertblock}
